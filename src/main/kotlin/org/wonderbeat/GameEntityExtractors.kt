...
fun <T> Boolean.then(funct: () -> T): T? = if (this == true) {
    funct()
} else null
...
fun Entity.extractItem(): Item? = (this.dtClass.dtName.contains("_Item_") &&
        ...
        this.dtClass.dtName != "CDOTA_Item_Physical") // IDK what is this item
        .then {
            Item(this.serial,
                    this.dtClass.dtName.substringAfterLast('_'),
...
class HeroAssembler {
    val heroToItems = TCustomHashMap<Hero, Set<Item>>(object : HashingStrategy<Hero> {
        override fun equals(p0: Hero?, p1: Hero?): Boolean = p0?.id == p1?.id
        override fun computeHashCode(p0: Hero?): Int = p0!!.id
    })
    ...
    fun assemble(frame: GameEntityFrame): GameEntityFrame {
        (frame.created + frame.updated).filterIsInstance<Hero>().forEach {
            heroToItems.putIfAbsent(it, setOf())
            ownerIdToHero.put(it.itemsOwnerId, it)
        }
        (frame.created + frame.updated).filterIsInstance<Item>().forEach { item ->
            heroToItems.put(ownerIdToHero[item.itemsOwnerId], heroToItems[ownerIdToHero[item.itemsOwnerId]]!! + (item))
        }
